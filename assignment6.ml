...
let rec substitute s t = match t with
				| Const s -> Const s
				| Var a -> let ispresent e = (match e with (v,n,t) -> if (v,n)=a then true else false) in
...
let rec mgu t1 t2 = match t1 with
					| Var (a,n) -> if t1=t2 then [] else (let rec search l e = (match l with hd::tl -> (if hd=e then true else false) | [] -> false) in
							if not (search (vars t2) (a,n)) then [(a,n,t2)] else raise NotUnifiable)
...
let rec evaluate goals program remrules subs = match goals with
					| [] -> subs
					| currgoal::gl ->  let result = [] in
						let rec goalloop prg rem goal goals i = (match rem with
							| cl::tl -> let sigma = (match cl with | Fact h -> (unify h goal) | Rule (h,b) -> (unify h goal) | _ -> raise TypeMismatch) in
										let remgoals = matchgoal cl goal goals in (match remgoals with
											...
											| _ -> result@(evaluate remgoals prg prg (compose sigma subs)) )
							| [] -> result) in (goalloop program program currgoal gl 0);;
...
unify ruleh remgoal2;;
evaluate remgoals1 prgm prgm pass;; 
evaluate [goal1] prgm prgm [];;
evaluate [goal2] prgm prgm [];;
